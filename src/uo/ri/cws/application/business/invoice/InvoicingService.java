...
public interface InvoicingService {
	 ...
	 * 
	 * @param the
	 *            list of workorder ids to bill
	 * @throws BusinessException
	 *             if - the state of any of the workorders is not FINISHED - any
	 *             of the workorders does not exist
	 * @throws IllegalArgumentException
	 *             if list is null, empty or any of the strings in the list is
	 *             empty or null
	 */
	 ...
	 *            dni
	 * @throws BusinessException
	 *             if - client with this dni does not exist
	 ...
	 * @param charges,
	 *            a List of ChargeDto whose: - Key (String) represents the
	 *            payment mean id, and - Value (Double) represents the amount to
	 *            be charged to the payment mean
	 * @throws IllegalArgumentException
	 *             if - invoiceId is null or empty - charges is null
	 * @throws BusinessException
	 *             if - there is no invoice for the invoiceId provided - the
	 *             indicated invoice is already in PAID state - does not exist
	 *             any of the payment means indicated by the id - the addition
	 *             of all amounts charged to each payment mean does not equals
	 *             the amount of the invoice with a precision of two cents (
	 *             Math.abs( total - amount) <= 0.01 ) - any of the payment
	 *             means cannot be used to pay the specified amount: - For a
	 *             CreditCard, if the current date is after the validThough date
	 *             - For a Voucher, if it has not enough available for the
	 *             amount - For Cash there is no constraint, cash can be used
	 *             always
	 *
	 *             Note (JUST FOR JPA IMPLEMENTATION): the domain model does not
	 *             have the proper design to do it polymorphically,
	 *
	 *             THUS
	 *
	 *             add a public abstract boolean canPay( amount ); method to
	 *             PaymentMean class and the proper specialization on the child
	 *             classes
	 */
